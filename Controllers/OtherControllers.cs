        ...
        if (r.ProductId.HasValue)
        {
            ...
            if (product is not null)
            {
                var approvedReviews = await db.Reviews
                    .Where(rv => rv.ProductId == r.ProductId && rv.Approved)
                    .ToListAsync();

                product.Rating = approvedReviews.Count > 0
                    ? Math.Round((decimal)approvedReviews.Average(rv => rv.Rating), 1)
                    : 0;
...
public class CouponsController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Validate(ValidateCouponRequest req)
    {
        ...
        discount = Math.Min(discount, req.CartTotal);
...
public class DashboardController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Get()
    {
        ...
        var salesRaw = await db.Orders
            .Where(o => o.CreatedAt >= since && (o.Status == "completed" || o.Status == "delivered"))
