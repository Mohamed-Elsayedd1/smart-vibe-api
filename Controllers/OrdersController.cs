...
public class OrdersController(AppDbContext db) : ControllerBase
{
    ...
    {
        // ─── حساب الـ subtotal مرة واحدة بس ──────────────────
        ...
        if (!string.IsNullOrWhiteSpace(req.CouponCode))
        {
            ...
            // تأكد الخصم مش أكبر من الـ subtotal
            discount = Math.Min(discount, subtotal);
            coupon.UsedCount++;
        }

        var shippingFee = subtotal > 1000 ? 0 : 50;
        ...
        foreach (var item in req.Items.Where(i => i.ProductId.HasValue))
        {
            var product = products.FirstOrDefault(p => p.Id == item.ProductId!.Value);
            if (product is null) continue;
            if (product.Stock < item.Quantity)
                return BadRequest(new { message = $"المنتج \"{product.Name}\" غير متوفر بالكمية المطلوبة (متاح: {product.Stock})" });
        }

        foreach (var item in req.Items.Where(i => i.ProductId.HasValue))
        {
            var product = products.First(p => p.Id == item.ProductId!.Value);
